-- Expand evidence-only document delivery events to contracts.
-- Contract support is manual/internal/print evidence only. Contract send,
-- signature, readiness, notification, and portal behavior remain owned by the
-- existing contract-specific workflows.
...
begin
  ...
  elsif new.subject_type = 'contract' then
    select contract.company_id
      into subject_company_id
    from public.contracts contract
    where contract.id = new.subject_id;
  else
...
comment on table public.document_delivery_events is
  'Immutable evidence-only delivery proof events for canonical documents. Current subject support is warranty_document, estimate, invoice, and contract.';
comment on column public.document_delivery_events.subject_type is
  'Canonical document subject namespace. Current support is warranty_document, estimate, invoice, and contract; contract support is manual evidence only.';
