...
export function ScheduleContextMetrics({
  ...
  return (
    <div
      className={`grid gap-3 ${columns === 2 ? "grid-cols-2" : "grid-cols-3"}`}
    >
      {items.map((item) => (
        <div
          key={item.label}
          className="rounded-[6px] border border-[var(--border-warm)] bg-[var(--highlight)] px-4 py-3"
        >
...
export function ScheduleContextFocusCard({
  ...
  return (
    <div className="rounded-[6px] border border-[var(--border-warm)] bg-white px-4 py-4">
      <p className="text-[10px] font-semibold uppercase tracking-[0.16em] text-[var(--text-secondary)]">
        {eyebrow}
      </p>
...
function getNoticeClassName(tone: "neutral" | "warning" | "positive") {
  switch (tone) {
    ...
    default:
      return "border-dashed border-[var(--border-warm)] bg-[var(--highlight)]";
  }
}
...
export function ScheduleContextNoticeBase({
  ...
  return (
    <div
      className={`rounded-[6px] border px-4 py-4 text-sm leading-6 ${getNoticeClassName(tone)}`}
    >
      {eyebrow ? (
        <p
      ...
      <div
        className={`${eyebrow || title ? "mt-2" : ""} text-[var(--text-secondary)]`}
      >
        {children}
      </div>
    </div>
...
function getActionClassName(
  variant: ScheduleContextAction["variant"] = "default"
) {
  if (variant === "subtle") {
...
export function ScheduleContextActions({
  actions
}: {
  actions: ScheduleContextAction[];
}) {
  return (
