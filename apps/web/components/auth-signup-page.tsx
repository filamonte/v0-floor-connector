...
export async function AuthSignupPage({ searchParams }: AuthSignupPageProps) {
  ...
  return (
    <AuthShell
      ...
      description={`Get started with FloorConnector to manage your ${surfaceContext.surfaceKey === "portal" ? "customer portal" : surfaceContext.surfaceKey === "superAdmin" ? "admin dashboard" : "connected contractor workflow"}. Founder early access stays operator-reviewed before production activation.`}
      error={params.error}
    ...
    >
      <div className="space-y-6">
        ...
        <form action={signUpAction} className="space-y-5">
          ...
          <div className="flex items-start gap-3 rounded-xl border border-amber-200 bg-amber-50 px-4 py-3">
            ...
            <p className="text-xs leading-5 text-amber-800">
              If email confirmation is enabled, you&apos;ll need to verify your
        ...
        <p className="text-center text-xs leading-5 text-[var(--text-tertiary)]">
          Founder access is reviewed before production activation. Terms and
          privacy paperwork are handled during operator review.
        </p>
      </div>
    </AuthShell>
  );
