...
import {
  getEstimateQuickCreateOptions,
  getEstimatesManagerReadModel,
  isEstimatesManagerView
} from "@/lib/estimates/manager-read-model";
...
export default async function EstimatesPage({
  ...
  const statusFilter = isEstimatesManagerView(resolvedSearchParams.status)
  ...
  const [readModel, quickCreateOptions, workflowSettings, financialSettings] =
    await Promise.all([
      getEstimatesManagerReadModel({
      ...
      showComposer
        ? getEstimateQuickCreateOptions(organizationContext.organization.id)
        : Promise.resolve({
            opportunities: [],
            customers: [],
            projects: []
  ...
  const draftCount = readModel.counts.draft;
  const sentCount = readModel.counts.sent;
  const approvedCount = readModel.counts.approved;
  const totalPipelineValue = readModel.pipelineValue;
  const filteredEstimates = sortEstimateRecords(readModel.estimates, sort);
  const estimateViews = [
    { key: "all", label: "All estimates", count: readModel.counts.all },
    ...
    { key: "approved", label: "Approved", count: approvedCount },
    {
      key: "rejected",
      label: "Revision",
      count: readModel.counts.rejected
    }
  ] as const;
  const draftQueue = readModel.draftQueue;
  const followUpQueue = readModel.followUpQueue;
  const approvedQueue = readModel.approvedQueue;
  const revisionQueue = readModel.revisionQueue;
  const recentClientResponses = readModel.recentClientResponses;
  const statusBreakdownTotal = Math.max(readModel.counts.all, 1);
  ...
  return (
    <ContractorWorkspacePage
      ...
      commandBar={{
        supportSlot: (
          <p>
            Start from the customer account, choose or create the project, then
            add the estimate. Opportunities can be linked as upstream context
            without becoming a second estimate model.
          </p>
        ),
        searchSlot: (
            ...
            <form action="/estimates" className="flex items-center gap-2">
              ...
              {resolvedSearchParams.opportunityId ? (
                <input
                  type="hidden"
                  name="opportunityId"
                  value={resolvedSearchParams.opportunityId}
                />
              ) : null}
              ...
              {resolvedSearchParams.projectId ? (
                <input
                  type="hidden"
                  name="projectId"
                  value={resolvedSearchParams.projectId}
                />
              ) : null}
      ...
      >
        <section className="flex flex-col gap-3">
          <div className="order-1">
            <EstimateRecordsPanel
              ...
              totalEstimateCount={readModel.counts.all}
              storageKey={ESTIMATES_ROWS_PER_VIEW_STORAGE_KEY}
              createHref={
                buildEstimatesHref({
                  q: query,
        ...
        >
          <EstimateQuickCreateForm
            ...
            opportunities={quickCreateOptions.opportunities}
            customers={quickCreateOptions.customers}
