...
import {
  getPeopleManagerReadModel,
  isPeopleManagerView
} from "@/lib/people/manager-read-model";
import { listPortalAccessGrants } from "@/lib/portal-access/data";
...
export default async function PeoplePage({ searchParams }: PeoplePageProps) {
  ...
  if (!organizationContext) {
    return (
      <section className="rounded-3xl border border-amber-200 bg-amber-50 px-8 py-6 text-sm leading-6 text-amber-900">
        Workforce records need an active organization before they can be
  ...
  const view = isPeopleManagerView(resolvedSearchParams.view)
  ...
  const selectedAccessContactId =
    resolvedSearchParams.accessContactId?.trim() ?? "";
  const selectedAccessGrantId =
    resolvedSearchParams.accessGrantId?.trim() ?? "";
  ...
  const [readModel, customerContacts, portalAccessGrants] = await Promise.all([
    getPeopleManagerReadModel({
      organizationId: organizationContext.organization.id,
      view,
      query,
      includeComposerOptions: showComposer
    }),
    listCustomerContactsForDirectory("/people"),
    listPortalAccessGrants()
  ]);
  const peopleViews = [
    { key: "all", label: "All people", count: readModel.counts.all },
    { key: "employees", label: "Employees", count: readModel.counts.employees },
    {
      key: "subcontractors",
      label: "Subcontractors",
      count: readModel.counts.subcontractors
    },
  ...
  return (
    <ContractorWorkspacePage
      ...
      summary={
        <div className="grid gap-2 sm:grid-cols-3 xl:grid-cols-3">
          ...
          <div className="border border-[#e5e5e5] bg-white px-4 py-3">
            ...
            <p className="mt-1 text-2xl font-semibold tracking-tight text-[#171717]">
              {readModel.counts.subcontractors}
            </p>
          </div>
          <div className="border border-[#e5e5e5] bg-white px-4 py-3">
            <p className="text-[11px] uppercase tracking-[0.14em] text-[#666666]">
      ...
      commandBar={{
        supportSlot: (
          <p>
            Search the workforce roster, switch between employee and
            subcontractor views, and use the customer-access section for portal
            invite status, linked contacts, and project visibility.
          </p>
        ),
        searchSlot: (
        ...
        actionSlot: (
          <Link
            href={
              buildPeopleHref({ q: query, view, compose: "1" }) +
              "#person-create"
            }
    ...
    >
      ...
      >
        <section className="space-y-6">
            ...
            <div className="border-b border-[#e5e5e5] px-5 py-4 sm:px-6">
              <div className="flex items-end justify-between gap-4">
                ...
                <p className="text-sm leading-6 text-slate-500">
                  {readModel.people.length} visible
                </p>
              </div>
            </div>

            <div className="divide-y divide-slate-200">
              {readModel.people.length > 0 ? (
                readModel.people.map((person) => (
                  <Link
                    key={person.id}
                    ...
                    <div className="grid gap-4 md:grid-cols-[minmax(0,1.2fr)_220px_180px] md:items-start">
                      <div className="min-w-0">
                        ...
                        <p className="mt-2 text-sm leading-6 text-slate-500">
                          {person.email ??
                            person.phone ??
                            "No direct contact added yet"}
                        </p>
                        <p className="mt-1 text-sm leading-6 text-slate-500">
                          {person.jobTitle ??
                            person.trade ??
                            "Role details not set"}
                        </p>
                      </div>
                      <div>
                      ...
                      <div className="md:text-right">
                        ...
                        <span className="inline-flex rounded-[4px] border border-[#d6d6d6] bg-[#f8f8f8] px-3 py-1 text-xs font-semibold uppercase tracking-[0.14em] text-slate-700">
                          {readModel.complianceCountByPersonId.get(person.id) ??
                            0}{" "}
                          record
                          {(readModel.complianceCountByPersonId.get(
                            person.id
                          ) ?? 0) === 1
                            ? ""
                            : "s"}
                  ...
                  <AppEmptyState
                    eyebrow={
                      readModel.counts.all > 0
                        ...
                        : "No workforce records yet"
                    }
                    title={
                      readModel.counts.all > 0
                        ? "Adjust the workforce filters"
          ...
          <PeoplePortalAccessPanel
            ...
            portalProjectAccessByGrantId={
              readModel.portalProjectAccessByGrantId
            }
            portalPermissionsByCustomerContactId={
              readModel.portalPermissionsByCustomerContactId
            }
            projectsByCustomerId={readModel.projectsByCustomerId}
        ...
        <WorkspaceComposerSheet
          ...
          open={showComposer}
          openHref={
            buildPeopleHref({ q: query, view, compose: "1" }) + "#person-create"
          }
        ...
        >
          <PersonForm
            ...
            vendors={readModel.vendorOptions}
            members={readModel.memberOptions}
          />
        </WorkspaceComposerSheet>
      </div>
    </ContractorWorkspacePage>
