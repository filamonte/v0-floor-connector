export type AppointmentConfirmationPreviewInput = {
  title: string;
...
export type AppointmentConfirmationPreview = {
  subject: string;
  body: string;
};
...
export function buildAppointmentConfirmationPreview(
  input: AppointmentConfirmationPreviewInput
  ...
  const subject = `Appointment confirmation: ${input.title}`;
  const body = compactLines([
    `Hi${input.customerName ? ` ${input.customerName}` : ""},`,
    "",
    `${organizationName} has confirmed your ${appointmentLabel} appointment.`,
    `Appointment: ${input.title}`,
    input.projectName ? `Project: ${input.projectName}` : null,
    `Status: ${labelize(input.status)}`,
