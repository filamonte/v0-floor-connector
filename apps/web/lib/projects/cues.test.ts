import assert from "node:assert/strict";
import test from "node:test";

import { buildProjectCues, selectHighestPriorityProjectCues } from "./cues";

const project = {
  id: "project-1",
  name: "Garage floor"
};

function readiness(overrides: Record<string, unknown>) {
  return {
    status: "ready_to_schedule",
    blockers: [],
    isReadyToSchedule: true,
    isOperationallyActive: false,
    depositRequired: false,
    depositSatisfied: true,
    financingStatus: "not_applicable",
    opportunityId: null,
    siteAssessmentStatus: null,
    estimateId: null,
    estimateStatus: null,
    contractId: null,
    contractStatus: null,
    contractInternalApprovalStatus: null,
    contractSignedAt: null,
    depositInvoiceId: null,
    depositInvoiceStatus: null,
    ...overrides
  } as unknown as NonNullable<Parameters<typeof buildProjectCues>[0]["readinessSnapshot"]>;
}

void test("project cues link approved estimates into existing contract generation", () => {
  const cues = buildProjectCues({
    project,
    readinessSnapshot: null,
    estimates: [
      {
        id: "estimate-1",
        status: "approved",
        referenceNumber: "EST-100",
        updatedAt: "2026-05-01T10:00:00.000Z"
      }
    ],
    contracts: [],
    invoices: [],
    jobs: []
  });

  assert.equal(cues[0]?.id, "project-1:approved-estimate-missing-contract");
  assert.equal(cues[0]?.href, "/contracts?estimateId=estimate-1");
  assert.equal(cues[0]?.actionLabel, "Generate contract");
  assert.equal(cues[0]?.workItemBridge, undefined);
});

void test("project cues link ready signed work into canonical job quick-create", () => {
  const cues = buildProjectCues({
    project,
    readinessSnapshot: readiness({
      status: "ready_to_schedule",
      isReadyToSchedule: true,
      estimateId: "estimate-1",
      contractId: "contract-1",
      contractStatus: "signed"
    }),
    estimates: [
      {
        id: "estimate-1",
        status: "approved",
        referenceNumber: "EST-100"
      }
    ],
    contracts: [
      {
        id: "contract-1",
        status: "signed",
        estimateId: "estimate-1",
        title: "Garage contract"
      }
    ],
    invoices: [],
    jobs: []
  });

  const cue = cues.find((item) => item.id === "project-1:signed-contract-no-job");

  assert.ok(cue);
  assert.equal(
    cue.href,
    "/jobs?projectId=project-1&compose=1&estimateId=estimate-1&contractId=contract-1"
  );
  assert.equal(cue.actionLabel, "Create job");
  assert.equal(cue.workItemBridge, undefined);
});

void test("project cues link ready unscheduled jobs into existing schedule action without work item bridge", () => {
  const cues = buildProjectCues({
    project,
    readinessSnapshot: readiness({
      status: "ready_to_schedule",
      isReadyToSchedule: true
    }),
    estimates: [],
    contracts: [],
    invoices: [],
    jobs: [
      {
        id: "job-1",
        dispatchStatus: "unscheduled"
      }
    ]
  });

  const cue = cues.find((item) => item.id === "project-1:ready-unscheduled-jobs");

  assert.ok(cue);
  assert.equal(cue.title, "Ready project needs scheduling");
  assert.equal(
    cue.href,
    "/schedule?projectId=project-1&view=unscheduled&action=schedule&jobId=job-1"
  );
  assert.equal(cue.actionLabel, "Open scheduling");
  assert.equal(cue.workItemBridge, undefined);
});

void test("deposit invoice project cue routes to invoice without unsafe work item bridge", () => {
  const cues = buildProjectCues({
    project,
    readinessSnapshot: readiness({
      status: "waiting_on_deposit",
      blockers: ["deposit_required"],
      isReadyToSchedule: false,
      depositRequired: true,
      depositSatisfied: false,
      depositInvoiceId: "invoice-1",
      depositInvoiceStatus: "sent"
    }),
    estimates: [],
    contracts: [],
    invoices: [
      {
        id: "invoice-1",
        status: "sent",
        workflowRole: "deposit",
        balanceDueAmount: "500.00",
        referenceNumber: "INV-100"
      }
    ],
    jobs: []
  });

  const cue = cues.find((item) => item.id === "project-1:deposit-invoice-unpaid");

  assert.ok(cue);
  assert.equal(cue.href, "/invoices/invoice-1");
  assert.equal(cue.actionLabel, "Review deposit invoice");
  assert.equal(cue.workItemBridge, undefined);
});

void test("open blocker field-note project cue keeps source-locked human follow-up bridge", () => {
  const cues = buildProjectCues({
    project,
    readinessSnapshot: null,
    estimates: [],
    contracts: [],
    invoices: [],
    jobs: [],
    fieldNotes: [
      {
        id: "note-1",
        dailyLogId: "daily-log-1",
        noteType: "blocker",
        status: "open",
        title: "Moisture issue"
      }
    ]
  });

  const cue = cues.find((item) => item.id === "project-1:open-blocker-field-notes");

  assert.ok(cue);
  assert.equal(cue.href, "/daily-logs/daily-log-1");
  assert.equal(cue.actionLabel, "Open daily log");
  assert.equal(cue.workItemBridge?.cue, "open_blocker_field_notes");
  assert.equal(cue.workItemBridge?.sourceType, "project");
  assert.equal(cue.workItemBridge?.sourceId, "project-1");
});

void test("ready project scheduling cue respects readiness and scheduled-job state", () => {
  const notReadyCues = buildProjectCues({
    project,
    readinessSnapshot: readiness({
      status: "waiting_on_deposit",
      blockers: ["deposit_required"],
      isReadyToSchedule: false
    }),
    estimates: [],
    contracts: [],
    invoices: [],
    jobs: [
      {
        id: "job-1",
        dispatchStatus: "unscheduled"
      }
    ]
  });
  const scheduledCues = buildProjectCues({
    project,
    readinessSnapshot: readiness({
      status: "ready_to_schedule",
      isReadyToSchedule: true
    }),
    estimates: [],
    contracts: [],
    invoices: [],
    jobs: [
      {
        id: "job-1",
        dispatchStatus: "scheduled"
      }
    ]
  });

  assert.equal(
    notReadyCues.some((cue) => cue.id === "project-1:ready-unscheduled-jobs"),
    false
  );
  assert.equal(
    scheduledCues.some((cue) => cue.id === "project-1:ready-unscheduled-jobs"),
    false
  );
});

void test("project cue preview keeps the highest-priority cues first", () => {
  const cues = selectHighestPriorityProjectCues(
    buildProjectCues({
      project,
      readinessSnapshot: readiness({
        status: "waiting_on_deposit",
        blockers: ["deposit_required"],
        isReadyToSchedule: true,
        estimateId: "estimate-1",
        contractId: "contract-1",
        depositRequired: true,
        depositSatisfied: false,
        depositInvoiceId: "invoice-1",
        depositInvoiceStatus: "sent"
      }),
      estimates: [],
      contracts: [],
      invoices: [
        {
          id: "invoice-1",
          status: "sent",
          workflowRole: "deposit",
          balanceDueAmount: "500.00",
          referenceNumber: "INV-100"
        }
      ],
      jobs: [
        {
          id: "job-1",
          dispatchStatus: "unscheduled"
        }
      ],
      fieldNotes: [
        {
          id: "note-1",
          dailyLogId: "daily-log-1",
          noteType: "blocker",
          status: "open",
          title: "Moisture issue"
        }
      ]
    }),
    2
  );

  assert.deepEqual(
    cues.map((cue) => cue.id),
    ["project-1:deposit-invoice-unpaid", "project-1:open-blocker-field-notes"]
  );
});
