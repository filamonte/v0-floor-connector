...
test("project detail renders decision-first UI", async ({ page }) => {
  ...
  await expect(page.getByRole("heading", { name: "Connected record lanes" })).toBeVisible();
  await expect(page.getByRole("heading", { name: "Sales / Estimate" })).toBeVisible();
  ...
  await expect(page.getByRole("heading", { name: "Job / Schedule" })).toBeVisible();
  await expect(page.getByRole("heading", { name: "Field / Daily Logs" })).toBeVisible();
  await expect(page.getByRole("heading", { name: "Customer Access" })).toBeVisible();
  ...
  await expect(page.getByText(/Full contact and account administration stays in People/i)).toBeVisible();
