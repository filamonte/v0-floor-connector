...
3. Done in part: Dashboard now has Operational Cockpit buckets that distinguish attention, ready handoff, waiting, and field/production follow-up. A bounded cockpit read-model pass narrows many bucket previews, a batched dashboard readiness helper reduces per-project readiness fan-out while preserving exact readiness semantics, and dashboard overview counts/current-person/appointment reads are now narrowed without changing visible create behavior.
