...
- Treat FloorConnector automation readiness as Ready With Human Review Gate:
  ...
  may not auto-merge, auto-continue to the next wave, mark PRs ready without
  review, or bypass Jeff approval.
...
- Use [docs/paid-early-access-plan.md](C:/FloorConnector/docs/paid-early-access-plan.md) before paid early-access work. The Phase 2.1 operating layer surfaces founder tenant setup/activation state and platform-admin-entered billing evidence in `/super-admin/early-access`; Phase 2.2 adds a test-mode-only FloorConnector SaaS subscription Checkout Session bridge from `/setup/billing`; Phase 2.3 adds a signed SaaS-only Stripe webhook reconciliation route for `billing_domain=floorconnector_saas` events; Phase 2.4 adds durable platform Billing Operations at `/super-admin/billing` for SaaS configuration health, Checkout readiness, webhook health, subscription references, manual evidence, and activation separation; Phase 2.5 adds non-secret `platform_billing_settings` and a platform-admin-only, test-mode-only Stripe Product/recurring Price create-or-discover action. Product/Price recovery requires a safely recognized `sk_test_` Stripe secret key, local test Checkout expects a `pk_test_` publishable key, and webhook signing secrets must remain env/provider-managed rather than stored in FloorConnector tables. Early access should remain a temporary commercial/readiness phase rather than the long-term billing IA. Use [docs/stripe-saas-billing-runbook.md](C:/FloorConnector/docs/stripe-saas-billing-runbook.md) for test-mode Product/Price setup, webhook endpoint setup, and replay. Use [docs/saas-billing-live-launch-plan.md](C:/FloorConnector/docs/saas-billing-live-launch-plan.md) before any live billing, Customer Portal, dunning, cancellation, entitlement, waiver, or production-release-gate work. Live subscription launch, automatic activation, entitlement enforcement, Customer Portal, and billing-state runtime gates still require dedicated approved implementation slices; subscription status is not activation truth or entitlement truth until an approved helper implements that policy.
- Use [docs/import-export-readiness.md](C:/FloorConnector/docs/import-export-readiness.md) before import/export work. The implemented slice is export-first and tenant-scoped at `/settings/export` for organization owners/admins. Exports are read-only views over canonical records, not snapshots or alternate source-of-truth records. Export history records metadata only, never exported rows or files. Customer/contact import dry run is validation-only: parse, map, validate, detect tenant-scoped duplicates, preview, and stop. Successful dry runs can now be saved into tenant-scoped `data_import_batches` / `data_import_rows` review records and opened through `/settings/export/imports/[batchId]`; those records store normalized preview and review metadata only, never raw uploads or created-record refs. Import writes remain disallowed until rollback, duplicate-resolution UX, backups, scoped permissions, row decisions, explicit owner/admin approval, and create/link-only execution are implemented through a separate approved phase. Update/merge behavior is deferred.
- SaaS billing reconciliation requires a canonical platform-wide `subscription_plans` catalog row before signed webhooks can create or update `company_subscriptions`. Configure the plan catalog through migration or an approved platform-admin path; never manually insert fake company subscription state as replay evidence.
