...
- Stripe SaaS replay prep on 2026-05-14 stayed names-only and stopped correctly before provider mutation: local Stripe key prefixes were not safely recognizable as test-mode, no app-managed platform billing price reference existed, `STRIPE_FOUNDER_PLAN_PRICE_ID` was missing, and `STRIPE_WEBHOOK_SECRET` was missing. No Product/Price action, Checkout Session, Stripe CLI forwarding, or webhook replay was run. The next replay can use the app-managed platform price reference after a platform admin runs the test-mode Product/Price setup action with a clearly test-mode Stripe key; `STRIPE_WEBHOOK_SECRET` is still required before webhook forwarding/replay.
