...
Recommended next slices: Manual/evidence-only contract delivery proof now exists after the reconciliation plan in [docs/contracts-delivery-proof-reconciliation-plan.md](C:/FloorConnector/docs/contracts-delivery-proof-reconciliation-plan.md), without touching signature truth. External e-sign adapter planning, provider-backed send architecture, change-order workspace maturity, and subcontract contract boundary planning remain later slices.
...
Current state summary: The first internal service/warranty continuity foundation is implemented through tenant-scoped `service_tickets`, `/service-tickets`, and `/service-tickets/:id`. It supports canonical customer, optional project, and optional original job linkage; source/type/status/priority classification; warranty dates/basis; description/resolution notes; manager/admin/owner mutation; active-member read access; and same-company relationship validation. Warranty document foundation is now implemented through warranty `document_templates`, default platform warranty seed adoption, tenant-owned warranty template editing in `/settings/templates`, canonical `warranty_documents`, `/warranty-documents/:id`, `/warranty-documents/:id/print`, `/portal/warranty-documents/:id`, and `/portal/warranty-documents/:id/print`. Generic signature groundwork now exists through `document_signers` and immutable `document_signature_events`, constrained to `warranty_document` subjects only. Warranty Document detail supports internal signer management plus request-signature audit events, and portal warranty review/signing now supports project-scoped customer review, print/save, email-matched signer validation, sign/decline actions, signer status updates, immutable generic signature events, and document `signed` status after all active customer signers sign. Immutable `document_delivery_events` now provide delivery history for warranty documents, estimates, invoices, and contracts; the supported document workspaces can record internal/manual/print evidence, and warranty documents, estimates, invoices, and the guarded contract send-for-signature path now support provider-backed portal email sends through `notification_events`, `notification_deliveries`, and `document_delivery_events`. Project, Customer, and Job Workspaces now show compact read-only continuity panels for linked service tickets, warranty documents, warranty date ranges, signer/request/signed counts, latest signature event summary, linked service jobs, and links back to canonical records. Portal Project Workspace shows customer-safe linked warranty documents without exposing service tickets. Dashboard Operational Cockpit now shows bounded read-only service/warranty signals for high-priority/stale tickets, tickets missing linked service jobs, unscheduled/upcoming/in-progress service jobs, and warranty documents needing internal signer/request attention. Service/warranty time now uses the canonical clocking foundation through optional `service_ticket_id` attribution on punch events and derived time cards, with Service Ticket detail showing linked time and routing to `/time` for capture. Service tickets with project context can create unscheduled canonical service jobs through `jobs.service_ticket_id`, so visits use the existing Schedule, Job Workspace, crew, equipment readiness, daily log, and time foundations. Portal service-ticket requests, provider callbacks, portal-visible delivery proof, countersign, provider e-sign, dispatch-grade service workflow, billing/manufacturer claims, job-costing mutation, and equipment/material usage automation remain future work.
...
Recommended next slices: Service/warranty architecture now lives in [docs/service-warranty-plan.md](C:/FloorConnector/docs/service-warranty-plan.md). Warranty document/signature architecture now lives in [docs/warranty-document-system-plan.md](C:/FloorConnector/docs/warranty-document-system-plan.md), with generalized send/signature findings in [docs/document-send-signature-architecture.md](C:/FloorConnector/docs/document-send-signature-architecture.md). Portal warranty review/signing is implemented from [docs/portal-warranty-review-sign-plan.md](C:/FloorConnector/docs/portal-warranty-review-sign-plan.md). Outbound document delivery and delivery proof is planned and partially implemented in [docs/document-delivery-proof-architecture.md](C:/FloorConnector/docs/document-delivery-proof-architecture.md), contract reconciliation lives in [docs/contracts-delivery-proof-reconciliation-plan.md](C:/FloorConnector/docs/contracts-delivery-proof-reconciliation-plan.md), and provider-backed send planning lives in [docs/provider-document-send-architecture.md](C:/FloorConnector/docs/provider-document-send-architecture.md). Provider-backed sends now cover warranty documents, estimates, invoices, and the guarded contract send-for-signature email path; contract-specific signature machinery still owns send/sign/view/decline/countersign truth.
...
Recommended next slices: Delivery-proof architecture now lives in [docs/document-delivery-proof-architecture.md](C:/FloorConnector/docs/document-delivery-proof-architecture.md). The `document_delivery_events` foundation now supports warranty documents, estimates, invoices, and contracts, with provider-backed email send implemented for warranty documents, estimates, invoices, and the guarded contract send-for-signature path. Contract delivery proof now follows [docs/contracts-delivery-proof-reconciliation-plan.md](C:/FloorConnector/docs/contracts-delivery-proof-reconciliation-plan.md), while `contract_signature_events`, notifications, portal views, provider callbacks, broader communication workflows, checkout/payment mutation from email delivery, and portal-visible delivery history remain separate until the evidence model is stable. Provider-backed outbound document send architecture now lives in [docs/provider-document-send-architecture.md](C:/FloorConnector/docs/provider-document-send-architecture.md). The next slice should audit provider-send behavior across all four core document types before adding callbacks or resend orchestration.

Not-now / deferred: Free-floating inbox disconnected from records.

## Portal / Customer Experience

Current state summary: Portal access, project visibility, estimate/contract/change-order/invoice review, contract signing, warranty document review/signing, and payment initiation/completion foundations exist.
...
Recommended next slices: Customer document center plan; portal service-ticket request/status planning; portal communication scope. Portal warranty review/signing now starts with project-linked issued warranty documents in the existing portal project workspace before broad customer document center or portal service-ticket request behavior.
