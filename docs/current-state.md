...
- a first shared universal-create launcher now exists in the shell and dashboard, routing into the existing module Quick-Create managers so new canonical records can be started broadly without creating a second creation system. Manual Universal Capture Phase 0 now adds a launcher handoff into `/dashboard?capture=1#universal-capture`, where a compact dashboard composer creates an internal Work Item through the existing `createWorkItemAction` path for clear follow-up/action intent.
...
- Scheduling / Field handoff visibility adds a read-only Field handoff layer to `/schedule`. The schedule page now derives crew assignment state, scheduled-day Daily Log presence, open field blocker/issue counts, field-note counts, same-day time-card counts, open time-card counts, latest field activity, and canonical Job / Project / Daily Log / Field queue links from existing `jobs`, `job_assignments`, `daily_logs`, `field_notes`, and `time_cards`. This is visibility only and does not add schema, migrations, dispatch records, mobile-only state, field task/issue tables, notifications, portal exposure, server actions, or schedule/field mutations.
...
- first lightweight internal work-item foundation:
  ...
  - Sales Handoff / Estimate Work Queue foundation now reuses those existing internal Work Items from the Lead Workspace. The Estimate Plan card can prefill an opportunity-linked estimate handoff item from real site-assessment state, requirements, notes, measurements, observations, and attachment counts. The prefill stores safe metadata for estimate work type (`generate_estimate`, `review_estimate`, `request_missing_info`, `approve_send`, or `follow_up_customer`), source opportunity, site-assessment packet counts, and planned role slots for onsite rep, relationship owner, estimate writer, follow-up owner, sales credit owner, and send-as user. The item is not created until a contractor submits the form.
  - `apps/web/lib/work-items/read-model.ts` now includes pure estimate-work queue selectors for assigned estimate work, waiting-on-me, ready-for-review, blocked estimates, and follow-ups due. Dashboard `My Work` consumes those selectors as explicit estimate handoff widgets, with rows showing the Work Item title, source link when available, assigned owner, requester when the creating user maps to an existing Person, due state, blocker reason, and next action metadata when it exists. Estimate Workspace also shows a compact Estimate Work panel for open estimate-production Work Items connected by estimate, project, or linked opportunity context; it can mark the existing Work Item in progress, save a blocker reason, complete the Work Item through existing assignee-or-manager Work Item actions, or use a compact Reassign control that updates only the existing tenant-scoped `work_items.assigned_person_id` value for an active assignable Person. Its rows distinguish assigned owner, requester, and waiting/blocked/review state from existing Work Item context only. The same panel has Request Missing Info and Request Review shortcuts that prefill the existing internal Work Item creation form with source-locked estimate context and `estimateWorkType` metadata; nothing is created until a contractor submits the form. Estimate Workspace now also shows a read-only Estimate Handoff Packet derived from the linked opportunity's site-assessment state, requirements, notes, reviewed measurement groups, observations, attachments, customer, project, and estimate links so estimate production can see what is captured and what is missing without adding persistence; source owner is shown only when the linked opportunity creator maps to an existing Person. Project Workspace now shows read-only estimate handoff continuity for open estimate-production Work Items connected to the project, its estimates, or linked opportunity; it summarizes open, blocked, ready-for-review, follow-up-due, next-most-urgent work, and compact assigned-owner/requester context, and routes back to the canonical Work Item, estimate, or source record. These are lenses/actions over existing `work_items` and opportunity/project/estimate records; they do not add schema, migrations, enum values, persisted workflow state, dashboard-owned records, project-owned task records, automatic work-item creation, estimate mutation, customer sends, notifications, portal behavior, payroll behavior, or commission calculation.
  ...
  - Manual Universal Capture Phase 0 reuses existing Work Items rather than adding `capture_items`: the shell Create menu links to `/dashboard?capture=1#universal-capture`, the dashboard shows a compact capture composer only for that handoff, and submission creates an unlinked internal Work Item with title, note, due/review date, assignee, priority, existing Work Item kind mapping, and safe metadata marking the capture source. Saved captures surface through the existing dashboard and Work Item queues.
  - The Phase 0 capture composer does not add schema, migrations, unresolved capture persistence, route/geographic grouping, assistant action staging, reminders, customer-facing sends/bookings, portal visibility, or mutations to customer/project/opportunity/job/invoice/communication/source records.
