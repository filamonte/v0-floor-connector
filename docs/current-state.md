...
- first lightweight internal work-item foundation:
  ...
  - `apps/web/lib/work-items/read-model.ts` now includes pure estimate-work queue selectors for assigned estimate work, waiting-on-me, ready-for-review, blocked estimates, and follow-ups due. Dashboard `My Work` consumes those selectors as explicit estimate handoff widgets, with rows showing the Work Item title, source link when available, owner/assignee, due state, blocker reason, and next action metadata when it exists. Estimate Workspace also shows a compact Estimate Work panel for open estimate-production Work Items connected by estimate, project, or linked opportunity context; it can mark the existing Work Item in progress, save a blocker reason, or complete the Work Item through existing assignee-or-manager Work Item actions. These are lenses/actions over existing `work_items`; they do not add schema, migrations, enum values, persisted workflow state, dashboard-owned records, automatic work-item creation, estimate mutation, customer sends, payroll behavior, or commission calculation.
