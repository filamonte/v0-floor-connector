...
- `/settings/company-documents` now exists as Company Documents Phase 1A: a contractor-owned Document Library for company administration documents such as SOPs, policies, agreements, safety plans, onboarding, training, and internal operations content. It is backed by the tenant-scoped `company_documents` table with forced RLS, active-member read access, owner/admin/manager mutation scope, archive/restore behavior, and settings-only create/edit forms. This is not `document_templates`, Document Engine output, legal advice, AI drafting, e-sign, employee/customer portal distribution, storage upload, provider sending, public links, Starter Documents, or platform-admin management.
