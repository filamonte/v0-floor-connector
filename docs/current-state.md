...
- `/settings/company-documents` now exists as Company Documents Phase 1B: a contractor-owned Document Library for company administration documents such as SOPs, policies, agreements, safety plans, onboarding, training, and internal operations content. It is backed by the tenant-scoped `company_documents` table with forced RLS, active-member read access, owner/admin/manager mutation scope, archive/restore behavior, settings-only create/edit forms, a contractor-side readable detail route at `/settings/company-documents/:id`, and a browser print/save route at `/settings/company-documents/:id/pdf`. This is not `document_templates`, legal advice, AI drafting, e-sign, employee/customer portal distribution, storage upload, provider sending, public links, delivery proof, Starter Documents, or platform-admin management.
