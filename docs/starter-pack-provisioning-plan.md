...
- `contractor_groups` and `contractor_group_memberships` store platform-owned contractor segmentation metadata and explicit organization membership for read-only targeting previews. They are not tenant roles, contractor permission groups, entitlements, pricing packages, module gates, or auto-provisioning triggers.
