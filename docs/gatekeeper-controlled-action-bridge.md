...
The schedule-site-assessment planning document lives in [docs/gatekeeper-schedule-site-assessment-controlled-execution-plan.md](C:/FloorConnector/docs/gatekeeper-schedule-site-assessment-controlled-execution-plan.md). It confirms that `schedule_site_assessment` remains non-executing today and recommends a future first execution model that updates opportunity site-assessment scheduled state through an Opportunities-owned helper before any appointment, job, schedule, message, or customer-visible behavior is introduced.
...
The current planning recommendation is that the first future `schedule_site_assessment` execution should not create appointments or jobs. Opportunities already own site-assessment scheduled/completed fields, and the schedule read model can surface opportunity-level scheduled assessments, so the safer first mutation is an Opportunities-owned site-assessment scheduled-state update with explicit date/time confirmation and ledger result linkage.
