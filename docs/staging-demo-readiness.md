...
Latest portal stream reconciliation note:

- `stream/portal` was reconciled with `origin/stream/portal` and `origin/main`,
  pushed through `1a4e2e4a chore: harden portal qa continuity`, and aligned
  with `origin/stream/portal`.
- The focused portal hardening run kept the portal as a project-scoped customer
  surface over canonical estimates, contracts, invoices, payments, change
  orders, project guidance, and shared action hubs.
- Validation for that portal stream run passed web typecheck, web lint, portal
  helper tests, focused env-loaded portal E2E with 9 passed and 1 skipped, a
  changed-file Prettier check, `git diff --check`, and
  `git diff --cached --check`.
- This is local/stream QA evidence, not proof of staging deployment, live
  provider readiness, live Stripe payment, external e-sign replay, or new
  portal-owned state.
