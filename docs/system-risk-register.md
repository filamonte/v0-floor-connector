...
| Import writes are not built beyond dry-run/read-only review readiness | Contractors can now get core records out, validate customer/contact CSVs with tenant-scoped duplicate signals, save review batches, and reopen a read-only row review shell, but bringing data in still needs executable row decisions, write audit completion evidence, rollback execution, duplicate-resolution UX, backups, and explicit approval before writes are safe. | High | Keep `/settings/export` as read-only export plus validation-only import dry run and read-only batch review; any future write phase must remain create/link-only first with dedicated audit and created-only rollback. | Decide the row-decision and approval-readiness slice before any mutation work. | Gather source-system sample headers during contractor review, save review batches for operator discussion, and do not import production data. |
