...
| Import is not built beyond export-first readiness | Contractors can now get core records out with metadata-only export history, but bringing data in still needs validation, mapping, duplicate detection, audit, and rollback before writes are safe. | High | Keep `/settings/export` as read-only tenant-scoped portability and treat import as validation/no-mutation planning until approved. | Decide first import dry-run scope, likely customers/contacts/projects from generic CSV. | Gather source-system sample headers during contractor review without importing production data. |
