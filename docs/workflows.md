...
- Current cue visibility remains organization-wide even when cues resolve to a responsible person or linked app user. Project-level overrides, record-level overrides, and user-specific My Work filtering are deferred. `sales_owner` and `field_lead` are intentionally deferred.
...
- Responsible role defaults are display/resolution metadata, not assignment state. Operational cues do not assign work to a person or user and do not filter My Work by user yet.
