...
- CrewBoard is the current scheduling visibility/action surface on `/schedule`;
  ...
  of truth. CrewBoard surfaces those derived warnings with compact card
  badges/counts and selected-job detail copy for Missing crew, Missing end time,
  Time overlap, Capacity warning, and readiness-blocked schedule context without
  persisting warning rows or promising notification delivery. CrewBoard can also
  show read-only Onsite Rep and Relationship Owner chips from explicit project
...
- `/portal` and portal Project Workspace now act as the customer hub over the
  same shared project chain: customers see customer-safe project status,
  estimate/contract/invoice cues, the next customer step, shared documents, and
  existing review/sign/pay/print paths without seeing contractor-only
  intelligence, readiness internals, field notes, provider metadata, or
  portal-only copies
...
- Create: Internal contractor work items can be manually created and optionally linked to a canonical source record. The current contractor UI supports explicit creation from a lead workspace against the current opportunity, from an appointment workspace against the current appointment, from a project workspace against the current project or selected project-level human follow-up context such as open blocker field notes, from a job workspace against the current job, and from estimate or invoice workspaces against the current record. Selected deterministic estimate/invoice operational cues can prefill source-locked work-item context, the Lead Workspace can prefill an estimate handoff item from real opportunity site-assessment, Scope Intake, requirements, measurement, observation, and attachment context, and the Estimate Workspace can prefill request-missing-info or request-review handoff items from the current estimate context. When the Lead Workspace already has an open estimate handoff for the same opportunity/project/estimate context, it points users back to that existing Work Item rather than preloading another estimate handoff draft. The user must submit the form.

- Track: Work items store internal ownership, due date, priority, status, assigned person, source link, and safe metadata on `work_items`. Context-Rich Work Items v1 reuses `description` for instructions/job notes and `metadata.measurementNotes` for measurement context, then renders that context from the shared work-item list. Estimate handoff work uses the existing internal Work Item record with `kind = estimate_follow_up` plus safe metadata such as `estimateWorkType` (`generate_estimate`, `review_estimate`, `request_missing_info`, `approve_send`, or `follow_up_customer`), internal `estimateWorkStatus`, blocker reason, and next-action text. Role Slots Foundation V1 stores separate ownership metadata on canonical opportunity/project/estimate records: Onsite Rep, Relationship Owner, Follow-Up Owner, Sales Credit Owner, and Estimate Writer. Every handoff creates visible work; every visible work item has an owner option, source record, due-state nudge, next action, and blocker state. Lead Workspace now exposes "Assign Estimate Writer" as the estimating handoff action when site assessment is complete or estimating is the next move; before an estimate exists, that assignment is the owner on the existing opportunity-linked estimate Work Item, and existing open handoffs can be reassigned without creating a duplicate Work Item. Dashboard `My Work` shows estimate handoff lenses for assigned estimate work, waiting on me, ready for review, blocked estimate work, and customer follow-ups due from those same Work Items; rows link back to the Work Item and source record when available, and show assigned owner plus requester when `work_items.created_by` maps to an existing Person through `people.membership_user_id`. Estimate Workspace now acts as the bounded record-context action surface for open estimate handoff Work Items connected to the estimate, project, or linked opportunity: users can mark the existing Work Item in progress, mark eligible estimate-production work ready for internal review by setting `metadata.estimateWorkStatus = ready_for_review`, update the existing row's next action through `metadata.nextAction`, set or clear the existing `work_items.due_at` field, save a blocker reason, complete it through the same assignee-or-manager Work Item action policy, reassign the open Work Item to an active assignable Person in the current organization through the existing `work_items.assigned_person_id` field, or prefill a new request-missing-info or request-review Work Item through compact shortcuts, with assigned owner, requester, due-state nudge (`Overdue`, `Due today`, `Due soon`, `Later`, or `No due date`), and waiting/blocked/review state labeled from the same Work Item context. Estimate Workspace also shows a read-only handoff packet from the linked opportunity so site-assessment state, requirements, notes, measurement groups, observations, photos/files, customer/project links, source-owner fallback, and missing source context stay visible during estimate production without becoming separate task or workflow state. Project Workspace now summarizes estimate handoff continuity from the same Work Items for managers: open, blocked, ready-for-review, follow-up-due, next-most-urgent estimate-production work, compact due-state nudge, and compact assigned-owner/requester context stay visible from the project hub and link back to the Work Item, estimate, or source record without adding project-local task state. Work Item Photo / Evidence Attachment Support lets project/job-linked work items carry internal photos/files through `execution_attachments.subject_type = work_item`, the private `documents` bucket, and contractor-only signed preview links. Dashboard work-item visibility prefers the current user's linked active `people` record when available and falls back to open company work items when needed. `/field/work-items` now gives the linked assignee a mobile-first field queue grouped by blocked, overdue, today, upcoming, and recently completed Work Items.

- Complete: Open work items can be completed or dismissed from the dashboard, lead workspace, appointment workspace, project workspace, job workspace, estimate workspace, or invoice workspace. The mobile assignee detail can mark an assigned Work Item in progress or blocked through safe metadata while it remains open, and can mark it done with the existing completed status/timestamp plus an optional `metadata.completionNote`. Completed/dismissed work items are not reopened in V1. Field Work Item status changes do not mutate jobs, Daily Logs, Job Notes, invoices, contracts, portal grants, or source records.
...
- Calendar events are not the source of truth for estimate-production work. Calendar/appointments describe scheduled time; Work Items describe required internal work, accountable owners, blockers, review, and handoff follow-through tied back to canonical opportunity/project/estimate records.
...
- Estimate handoff Work Items and the Estimate Handoff Packet do not create estimates, send estimates, mutate opportunity/project readiness, create notification events, create commission/payroll obligations, or replace canonical estimate status. Internal ready-for-review updates are Work Item metadata only; they do not move `estimates.status` to sent, approved, or rejected. Next-action editing updates existing Work Item metadata only, and due-date editing updates the existing Work Item due-date field only; they do not create duplicate Work Items, reminders, notification sends, or messages. Lead Workspace duplicate-aware handoff creation is a read-model guard over existing open `work_items`, not new persistence. Reassignment updates only the existing canonical Work Item assignee after tenant-scoped active/assignable Person validation. Role-slot editing updates only canonical opportunity/project/estimate ownership metadata after tenant-scoped active/assignable Person validation; it does not infer or mutate Work Item assignment. The handoff packet is a derived display over existing opportunity/project/estimate context only. Owner clarity display uses existing assigned person, creating user to Person linkage, source opportunity creator linkage, and explicit role-slot metadata only; it does not infer legal commission owner, payroll responsibility, or payout behavior. Project Workspace estimate handoff continuity is read-only and does not add project-local Work Item actions. Estimate Workspace missing-info and review-request shortcuts prefill internal Work Item creation only; send-as workflow and commission calculation behavior remain planned. Sales Credit Owner is implemented as metadata only until a dedicated approved commercial/financial slice adds downstream calculation policy.
