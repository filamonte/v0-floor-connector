...
- Estimate handoff Work Items do not create estimates, send estimates, mutate opportunity/project readiness, create commission/payroll obligations, or replace canonical estimate status. Project Workspace estimate handoff continuity is read-only in V1 and does not add project-local Work Item actions. Estimate Workspace missing-info and review-request shortcuts prefill internal Work Item creation only; dedicated ready-for-review estimate status transitions, next-action editing on existing Work Items, send-as workflow, and commission owner behavior remain planned. Commission ownership remains planned metadata until a dedicated approved commercial/financial slice adds auditable canonical owner fields and downstream calculation policy.
