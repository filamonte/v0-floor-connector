...
- Estimate handoff Work Items and the Estimate Handoff Packet do not create estimates, send estimates, mutate opportunity/project readiness, create notification events, create commission/payroll obligations, or replace canonical estimate status. Internal ready-for-review updates are Work Item metadata only; they do not move `estimates.status` to sent, approved, or rejected. Next-action editing updates existing Work Item metadata only, and due-date editing updates the existing Work Item due-date field only; they do not create duplicate Work Items, reminders, notification sends, or messages. Lead Workspace duplicate-aware handoff creation is a read-model guard over existing open `work_items`, not new persistence. Reassignment updates only the existing canonical Work Item assignee after tenant-scoped active/assignable Person validation. The handoff packet is a derived display over existing opportunity/project/estimate context only. Owner clarity display uses existing assigned person, creating user to Person linkage, and source opportunity creator linkage only; it does not infer onsite rep, relationship owner, sales credit owner, legal commission owner, or payroll responsibility. Project Workspace estimate handoff continuity is read-only and does not add project-local Work Item actions. Estimate Workspace missing-info and review-request shortcuts prefill internal Work Item creation only; send-as workflow and commission owner behavior remain planned. Commission ownership remains planned metadata until a dedicated approved commercial/financial slice adds auditable canonical owner fields and downstream calculation policy.
