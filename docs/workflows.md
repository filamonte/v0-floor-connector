...
- Estimate handoff Work Items and the Estimate Handoff Packet do not create estimates, send estimates, mutate opportunity/project readiness, create notification events, create commission/payroll obligations, or replace canonical estimate status. Internal ready-for-review updates are Work Item metadata only; they do not move `estimates.status` to sent, approved, or rejected. Reassignment updates only the existing canonical Work Item assignee after tenant-scoped active/assignable Person validation. The handoff packet is a derived display over existing opportunity/project/estimate context only. Owner clarity display uses existing assigned person, creating user to Person linkage, and source opportunity creator linkage only; it does not infer onsite rep, relationship owner, sales credit owner, legal commission owner, or payroll responsibility. Project Workspace estimate handoff continuity is read-only and does not add project-local Work Item actions. Estimate Workspace missing-info and review-request shortcuts prefill internal Work Item creation only; next-action editing on existing Work Items, send-as workflow, and commission owner behavior remain planned. Commission ownership remains planned metadata until a dedicated approved commercial/financial slice adds auditable canonical owner fields and downstream calculation policy.
