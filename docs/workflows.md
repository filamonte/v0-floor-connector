...
- Estimate handoff Work Items and the Estimate Handoff Packet do not create estimates, send estimates, mutate opportunity/project readiness, create commission/payroll obligations, or replace canonical estimate status. The handoff packet is a derived display over existing opportunity/project/estimate context only. Owner clarity display uses existing assigned person, creating user to Person linkage, and source opportunity creator linkage only; it does not infer onsite rep, relationship owner, sales credit owner, legal commission owner, or payroll responsibility. Project Workspace estimate handoff continuity is read-only in V1 and does not add project-local Work Item actions. Estimate Workspace missing-info and review-request shortcuts prefill internal Work Item creation only; dedicated ready-for-review estimate status transitions, next-action editing on existing Work Items, send-as workflow, and commission owner behavior remain planned. Commission ownership remains planned metadata until a dedicated approved commercial/financial slice adds auditable canonical owner fields and downstream calculation policy.
