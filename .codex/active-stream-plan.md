...
- Status: Merged to `main` via PR #25.
...
- Owns: presentation-only mobile scanability for Lead Assessment Package,
  Schedule Field Handoff, Daily Logs, and assigned Field Work surfaces.
- Mission: make mobile assessment capture and field-facing handoffs easier to
  scan and tap while preserving the existing canonical Lead/Opportunity,
  Project, Job, Daily Log, Field Work, Communication, schedule, and readiness
  ownership model.
- Dependencies: PR #21 UX design-system foundation, PR #23 dashboard cleanup,
  PR #24 record workspace rhythm, PR #25 financial/schedule/production
  readiness UX, UX Beta Readiness packet, UX architecture audit packet,
  financial/schedule readiness packet, design-system governance, UI patterns,
  current-state truth, developer source of truth, and target IA.
- Forbidden: business logic changes, readiness calculation changes,
  schema/data changes, Supabase changes, migrations, duplicate field queues,
  mobile-only/offline persistence, dashboard-owned or field-local operational
  state, personalization engine, role-specific records, portal copies,
  financial/schedule/readiness truth changes, payment/signature/scheduling
